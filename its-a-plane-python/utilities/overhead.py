...
# Helicopter-only airports: anything touching one IS a helicopter, even if
# the type code is missing or an unrecognized variant (e.g. HeliFlite S76D
# out of JRA West 30th St).
HELIPORT_IATAS = {"JRA", "JRB", "TSS"}


def _is_helicopter_type(code):
    if not code:
        return False
    code = code.upper()
    if code in HELICOPTER_TYPES:
        return True
    # variant suffixes: S76D, EC35T, AW139 short forms etc.
    return any(code.startswith(t) for t in HELICOPTER_TYPES if len(t) >= 3)
...
def safe_write_json(path: str, data):
    ...
    # reported as a write failure (it spammed journals on all three Pis).
    try:
        os.chmod(path, 0o666)
    except OSError:
        pass
...
class Overhead:
    ...
    def _write_processing_flag(value):
        """Mirror contract: lets the web display blink the loading pulse."""
        try:
            import time as _time
            path = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                ".cache", "processing.json")
            safe_write_json(path, {"processing": bool(value), "ts": _time.time()})
        except Exception:
            pass
    ...
    def _grab(self):
        ...
        try:
            ...
            for f in flights:
                ...
                while retries:
                    try:
                        ...
                        if owner_icao != "HELI" and (
                                origin in HELIPORT_IATAS
                                or destination in HELIPORT_IATAS):
                            owner_icao = "HELI"
                            stats["helicopters"] += 1
            ...
            # Write current state for display mirror (with timestamps for scroll sync)
            try:
                from time import time as _now
                ts = _now()
                display_data = [{k: v for k, v in f.items() if k != "trail"}
                                for f in overhead_data]
                safe_write_json(os.path.join(DATA_DIR, "current_overhead.json"),
                                {"flights": display_data, "ts": ts})
                # Tracked flight (live data + nearest landmark for mirror)
                td = dict(tracked_data) if tracked_data else {}
                if td:
                    td["_mirror_ts"] = ts
                    # Add nearest landmark so browser can show "nr CityName"
                    lat = td.get("latitude")
                    lon = td.get("longitude")
                    if lat and lon:
                        try:
                            from utilities.landmarks import get_nearest_landmark
                            lm = get_nearest_landmark(lat, lon)
                            if lm:
                                td["_nearest_landmark"] = lm.get("name", "")
                        except Exception:
                            pass
                safe_write_json(os.path.join(DATA_DIR, "current_tracked.json"), td)
            except Exception:
                pass
            ...
            with self._lock:
                self._data = []
                # Keep the last tracked flight across a transient network
                # failure — nulling it made the display flap between the
                # tracked scenes and the forecast on every blip (the live
                # path has its own staleness handling via
                # estimate_stale_data / _MAX_TRACKED_HOURS).
                self._new_data = True
            try:
                safe_write_json(os.path.join(DATA_DIR, "current_overhead.json"), [])
            except Exception:
