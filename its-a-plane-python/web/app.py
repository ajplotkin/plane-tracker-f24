...
def api_config_get():
    ...
    for key in [
        ...
        if key in {"NIGHT_BRIGHTNESS", "HAT_PWM_ENABLED", "NWS_ALERTS_ENABLED", "ISS_ALERTS_ENABLED",
                   "HOURLY_CHIME_ENABLED", "ATC_ENABLED", "ATC_AUTO_RESUME", "AQI_ALERTS_ENABLED"}:
...
def api_display_state():
    ...
    # AQI: read the value the display process already fetched+cached (don't call
    # AirNow from the web process), gated by the same enable/threshold as the LED
    # chip so the mirror shows it exactly when the panel does.
    try:
        import config as _c
    ...
    return jsonify({
        ...
        "humidity": humidity,
        "uv_index": uv_index,
        "aqi": aqi,                   # gated EPA AQI for the colour chip (or None)
        "forecast": forecast,
